...
NETWORK_COUNT=$(echo "$PEEK_JSON" | jq '.data.network // [] | length')
...
if [ "$NETWORK_COUNT" -eq 0 ]; then
  ...
  SKIP_NETWORK=false
  FIRST_REQUEST_ID=$(echo "$PEEK_JSON" | jq -r '.data.network[0].requestId // empty')
...
CONSOLE_COUNT=$(echo "$CONSOLE_PEEK" | jq '.data.console // [] | length')
